...
## 疑似Bug先汇报铁律（2026-06-25 新增）
- 发现代码中可能存在的 bug 时，**不能直接动手改**。必须先：
  1. 列出你发现的可疑代码（文件和行号）
  2. 用日常语言解释：代码现在在做什么、你觉得哪里像 bug、如果改了会怎样
  3. **等用户确认再动手**
- 典型案例：C04 薛定谔的猫——代码里有个 `_c04PlayerChoose` 标记从未被使用，同时引擎用 `Math.random()` 随机选择效果。AI 认定随机是"未实现"并改成了弹窗，但用户说随机就是原本的设计。**代码未使用的标记不一定是 bug，可能是预留或废弃的设计**。
- 教训：2026-06-25 花了一次 revert 才明白——设计意图只有用户知道，AI 只能汇报不能替用户决定。
