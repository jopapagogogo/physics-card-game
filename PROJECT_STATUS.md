# 物理卡牌对战游戏 — 项目总览

> ⚠️ **给 AI 助手的指令**：
> - **开工第一步**：`git pull` 拉取最新代码和本文件。
> - 本文档是唯一权威的项目文档（进度 + 架构 + 规则 + 计划）。代码中可能存在未完成的功能痕迹，不在当前范围内，请忽略。
> - **完工最后一步**：更新本文件 → `git add -A && git commit -m "描述" && git push`。全部不需要等用户提醒。

---

## 项目定位

初中物理五大领域（力/声/光/热/电）卡牌对战游戏。开发者是零技术基础的物理教师，所有代码由 AI agent 生成。长期目标是触达真实玩家（学生/物理爱好者）。

## 技术栈

原生 HTML/CSS/JS，无框架，`index.html` 双击即运行。纯逻辑层（engine/cards/combo）与 UI 零耦合，未来可复用。

## 六大领域

| 领域 | 数量 | 颜色 | 关键词 |
|------|------|------|--------|
| 力 Force | 19 | 红 #E74C3C | 力学、运动、能量 |
| 电 Electric | 19 | 紫 #9B59B6 | 电流、电压、磁场 |
| 声 Sound | 20 | 绿 #16A085 | 声波、共振、频率 |
| 光 Light | 24 | 橙 #F39C12 | 光学、透镜、波粒 |
| 热 Heat | 20 | 暖橙 #E67E22 | 热力学、温变、相变 |
| 混沌 Chaos | 7 | 紫 #9333ea | 四神兽 + 相变卡 |

## 文件结构

```
物理卡牌游戏/
├── index.html              ← 游戏入口
├── all_cards.html          ← 109张卡牌展示页（静态）
├── PROJECT_STATUS.md       ← 本文件（唯一权威）
├── package.json            ← vitest 测试配置
├── css/game.css            ← 游戏主样式
├── js/
│   ├── cards.js            ← 109张卡牌数据（唯一权威源）
│   ├── engine.js           ← 核心战斗引擎
│   ├── combo_table.js      ← 35个 combo 查找表
│   ├── ai.js               ← AI 对手逻辑
│   ├── ui.js               ← 界面控制器
│   └── quiz.js             ← 物理题库
├── art_samples/card_art/   ← 109+ 张卡牌插画
├── generate_cards.py       ← 卡牌展示页生成器
├── domain_runes.json       ← 6领域符文 base64
├── approved_cards.json     ← 卡牌→插画文件映射（唯一权威）
└── tests/combo.test.js     ← 34个测试用例
```

---

## 工作规则（铁律）

1. `js/cards.js` 是唯一的卡牌数据权威源
2. 引擎实现效果时，effect 属性名必须直接使用 cards.js 中的字段名
3. `approved_cards.json` 是卡牌→插画的唯一映射，未审核确认不得写入
4. 插画配对唯一依据是 `approved_cards.json`，不允许根据文件名自动猜测
5. 工作资料三级分区：Confirmed（approved_cards.json）/ Pending（未审核）/ Deprecated（已淘汰）
6. **开工前 `git pull`，收工后 `git add -A && git commit && git push`**

---

## 已完成阶段

| 阶段 | 内容 | 状态 |
|------|------|:---:|
| Phase 1 | 109 张卡牌生成 | ✅ |
| Phase 2 | Combo 系统重建（35 combo + engine.js 重写） | ✅ |
| Phase 3 | 可验证 AI 对战测试（34 用例） | ✅ |
| Phase 4 | 7 种剩余 combo 效果 + 召唤被动 + UI 高亮 | ✅ |
| Phase 5 | 全部 109 张插画生成 + 领域符文 + 混沌领域 | ✅ |
| C03 | 拉普拉斯妖窥牌排序交互（引擎+UI模态框） | ✅ |
| Deck | 卡组构建器（固定30张，主12-18，副6-12，领域≤2） | ✅ |
| Quiz | 题库扩展 100→190 题（6领域含混沌，关联109张卡牌） | ✅ |
| P2 | 物理知识深度融入（答题影响费用 + Combo 物理原理展示） | ✅ |

---

## 🎯 当前优先级（Phase 6）

用户反馈的核心痛点，按重要性排序：

### P0：AI 策略升级 ✅ 完成
**完成内容**：AI 引擎全面重构（ai.js v2.0），7 大模块升级：
1. **ComboIndex** — 导入真实 COMBO_TABLE（35 个 combo）替代硬编码 KNOWN_COMBOS（11 个），支持支撑→攻击/攻击→攻击/召唤→攻击/召唤对冲/跨领域对抗五种模式
2. **ThreatAssessor** — 威胁评估系统（对手进攻力 + combo 潜力 + 己方脆弱度 + 驻场威胁 + DOT 累计）
3. **SpiritBudgetManager** — 精神力预算（full_send/balanced/save 三档，hard 难度精确分配）
4. **DefensePlanner** — 防守策略（HP 阈值 + 高威胁召唤物 + DOT 致命 + 关键召唤物保护）
5. **ComboDetector** — 多模式 combo 搜索（5 种模式，按评分排序）
6. **AttackEvaluator** — 增强攻击卡评估（引入威胁权重）
7. **MultiTurnPlanner** — 多回合规划（hard 难度蓄力→下回合爆发）
- **测试结果**：34 项全部通过，50 局 AI 对战 0 崩溃，combo 触发率 92%

### P1：出牌体验优化 ✅ 完成
**完成内容**：
1. **卡牌飞行动画** — 玩家出牌时卡牌从手牌区飞向战场出牌区（CSS transform动画，0.3-0.5秒），攻击卡使用弧线轨迹，辅助/领域/召唤卡直飞
2. **伤害数字弹出** — 攻击命中时伤害数字从目标位置弹出，红色（伤害）/绿色（回复）/粉色（combo额外）/橙色（暴击），scale+translateY+fadeOut动画约1秒
3. **Combo触发特效** — combo触发时战场中央显示combo名称，屏幕边缘金色闪光，文字放大→缩小→消失动画约1.6秒
4. **出牌区视觉层次** — 卡牌在出牌区按类型有不同边框高亮色（攻击红/辅助蓝/领域金/召唤紫/相变青），阴影+发光效果
5. **回合切换过渡** — 回合切换时500ms过渡动画，顶部滑入"你的回合"/"对手回合"横幅提示，背景半透明遮罩
6. **AI出牌视觉** — AI出牌时同样享受飞行和特效（伤害数字在己方弹出、AI combo也有标识）
- **实现方式**：纯CSS @keyframes + JS动态DOM创建，使用transform/opacity等GPU加速属性，移动端流畅，不引入第三方库，不改engine.js/ai.js，动画是纯视觉层不影响游戏逻辑

### P2：物理知识深度融入 ✅ 完成

**完成内容**：
1. **答题影响出牌费用（engine.js）**：
   - 答对N题 = 本回合N张卡各-1费（知识减费）
   - 答错N题 = 本回合每卡+N费（知识惩罚）
   - 构造函数新增 `quizCostReduction[]` 和 `quizCostPenalty[]` 状态数组
   - `setQuizResult()` 中计算费用影响并记录日志
   - `playCard()` 和 `canAfford()` 中应用费用调整
   - `startTurn()` 中每回合重置

2. **答题结果视觉反馈（ui.js）**：
   - 出牌时触发知识减费/加费，在手牌区显示绿色（减费）/红色（加费）徽章提示
   - 伤害计算时在战斗日志中显示 `[知识之力 +X%]` 标记

3. **Combo 物理原理突出展示（ui.js）**：
   - `_showComboEffect()` 重构：解析 combo msg 中的物理概念链
   - 物理概念用 22px 绿色大字展示，箭头分隔，效果描述用小字
   - 新增 `_parseComboMsg()` 解析方法（支持 →、↔、vs 三种分隔符）
   - 导出 combo 物理概念关键词（质量增大→重力锤击、红外灼烧→热辐射 等）

**改动文件**：
- `js/engine.js` — 构造函数 + setQuizResult + playCard + startTurn + canAfford + 伤害日志
- `js/ui.js` — _showComboEffect 重构 + _parseComboMsg 新增 + _showQuizCostFeedback 新增 + CSS 样式

**测试结果**：34 项全部通过 ✅

---

## 📅 长期路线图

| 阶段 | 时间 | 做什么 | 怎么触达玩家 |
|------|------|------|------|
| **打磨期** | 1-2 月 | AI 策略 + 出牌体验 + 题库扩充 + 平衡性 | 物理教研群内测链接 |
| **PWA 化** | 2-3 月 | 手机可装到桌面，离线可玩 | 一个网址发给学生 |
| **联网功能** | 3-6 月 | 双人对战 + 排行榜 | 学生间对打，自然传播 |
| **App 化** | 6 月后 | 根据真实数据评估是否换 Cocos/Unity | 应用商店上架 |

---

## Git

- 仓库：git@gitee.com:jopapa/physics-card-game.git
- 分支：master
